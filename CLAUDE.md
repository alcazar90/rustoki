...
3. Render each source through the Markdown pipeline (`render/mod.rs`), which walks `pulldown-cmark`'s event stream and intercepts:
   - **Headings** → auto `id=` anchors + collected into a Table of Contents (rendered inline as a `<nav class="toc">` block if a post has ≥2 headings, unless the post sets `toc: false` in its frontmatter; the anchors are emitted either way).
   ...
   - **Algorithm blocks** → `render/algorithm.rs`. A fence tagged `algorithm` becomes a `<figure class="algorithm">`: an optional `title:`/`label:` header above a `---` line, then one statement per line, indented to nest (depth is measured in units of the block's smallest indent). Each statement is fed back through `transform_events` via `render_inline_markdown` (the same trick the footnote bodies use), so math, code spans, emphasis and citations inside a line render exactly as in the prose; the module only adds the structure around it (line numbers, `alg-kw` spans for `for … do`/`if … then`/`end for`/`return`/…, small-caps names after `function`/`procedure`, `// comments`, unnumbered `Input:`/`Output:` lines). Every statement is wrapped in `<span class="alg-stmt">` *before* parsing: a line that opens with inline HTML followed by text can start no other CommonMark block, which is what stops `1. step` or `# comment` at the head of a line becoming a list or a heading. `<`/`>` are escaped outside math and code spans so a statement can never open a raw HTML tag; the anchors the build's own preprocessors leave behind (citations, `\ref`, `\figref`) are recognised and let through. Blocks are numbered in document order by an `Algorithms` counter threaded through the walk, and `\algref{key}` is resolved *after* rendering (`replace_algrefs`, skipping `<code>`) so the numbers are the ones the walk actually handed out. A header is only recognised when every line above the `---` is a known key (`title`/`label`), so an `Input:`/`Output:` preamble followed by a `---` rule, or a misspelt key, renders as statements rather than vanishing; nothing here fails the build.
...
4. Render through minijinja templates (`templates.rs`) — one context struct per page kind (`PostContext`, `PageContext`, `IndexContext`, `Render404Context`), each wrapping a shared `RenderEnv` (site config + inlined CSS + build year).
   Post pages also carry an older/newer link pair at the foot of the article (`PostNeighbour`, computed in `main.rs` from the same date-sorted list the home page shows, so drafts appear there only under `--drafts`); it is omitted entirely when a post has no neighbour.
   The home page also carries a decorative raked-sand garden (`sandgarden.rs`): a constant bed of hairline grooves with one stone per recent post (at most three), each placed by a hash of its own slug so it never moves, sized by reading time, and with the grooves bent into rake circles around it. It is inlined as an SVG behind the profile row, anchored to the right and faded in from the left by CSS, and nothing in it can fail the build. `garden = false` in the config leaves it out.
...
- Frontmatter (YAML `---` or TOML `+++`) supports: `title`, `date`, `slug`, `tags`, `description`, `draft`, `lang`, `toc`. `toc: false` suppresses the auto-generated table of contents for that one post (unset means auto). `draft: true` posts are skipped by a plain `build`; pass `--drafts` to render them locally (with a "Draft" badge on the post page and index listing) while still excluding them from `feed.xml`/`sitemap.xml`.
...
- Algorithms: a ```` ```algorithm ```` fence with an optional `title:` / `label:` header closed by a `---` line, then one statement per line, indented to nest. Lines are inline Markdown (math in `$…$`), keywords are recognised by position (`for`/`while`/`if`/`else`/`end …`/`repeat`/`until`/`function`/`procedure`/`return` at the head of a line, `do`/`then` at its tail, `Input:`/`Output:`/`Require:`/`Ensure:` with the colon), and `// text` at the end of a line is a comment. `\algref{label}` links to the block as "Algorithm N". Display math and block elements do not belong inside a statement.
